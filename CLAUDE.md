...
- **Act as Team Lead when spawning Agent Teams** — always spawn the Team Lead as a dedicated agent via the Agent tool (`team-lead`). The main agent's context window is reserved for user interaction, not team coordination overhead. See `guides/AGENT_TEAMS_GUIDE.md`
