...
**Use:** `/checklist/AI_CODE_SECURITY_REVIEW.md` for detailed security review.
...
- [ ] **Unicode/emoji:** Non-ASCII characters handled (e.g., "café", "👍")
...
- [ ] **Big O analyzed:** Is this O(n²) when O(n) exists?
...
### Hallucinated Code
- [ ] **APIs don't exist:** AI invented `array.findLast()` (pre-ES2023)
...
```markdown
**Correctness:** ✅ Logic looks correct for the happy path.

**Security:** ⚠️ SQL query uses string concatenation. Need parameterized query.

**Edge Cases:** ❌ Doesn't handle empty array. Will crash on line 42.

**Performance:** ⚠️ N+1 query issue. Consider eager loading.

**Tests:** ❌ No tests for error cases.

**Recommendation:** REVISION_REQUESTED

**Action Items:**
1. Fix SQL injection vulnerability (line 38)
2. Add null check for empty array (line 42)
3. Add error case tests
4. Consider refactoring DB query to avoid N+1
```
