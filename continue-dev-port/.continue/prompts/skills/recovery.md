...
> Note: These git commands work the same on all platforms. However, always confirm with the user before running destructive commands like `git reset --hard`.
> Note: Adjust commands for PowerShell on Windows if piping or quoting differs.
...
6. Update GitHub issue with abandon comment:
   ```bash
   gh issue comment NNN --body "Implementation abandoned. Recovery report: docs/recovery/YYYY-MM-DD-feature.md"
   ```
