...
Run git diff to inspect current changes:
...
> Note: Adjust commands for PowerShell on Windows (e.g., `git diff HEAD` works the same, but piping to search tools differs -- use `Select-String` instead of `grep`).
