---
alwaysApply: false
description: "Solution: Status lifecycle transitions must be forward-only — prevent accidental regression of plan/issue status fields"
module: Skills
date: 2026-02-14
...
symptoms:
  ...
  - "Re-running a loop on a finished plan silently reopens it"
...
- Running `start-issue` or a loop on a `complete` plan would reopen it as `in_progress`
- No warning when attempting backward transitions

## What Didn't Work

**Direct solution:** Caught by edge-case reviewer during a review. No failed attempts.
