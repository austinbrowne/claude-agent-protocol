...
## Problem
The GitHub label creation script in `guides/GITHUB_PROJECT_INTEGRATION.md` defined `status: ready` but every workflow skill used `ready_for_dev` instead. The two label names evolved independently with no cross-validation, causing silent failures for users who followed the setup guide.

## Environment
- Module: GitHub Integration (guides/GITHUB_PROJECT_INTEGRATION.md)
...
**Direct solution:** The problem was identified during a fresh-eyes-review when formalizing the issue lifecycle state machine into a transitions table. The mismatch was invisible in the individual files but obvious when all transitions were listed together.
...
- See also: [Direct Workflow Routing](../workflow-issues/direct-workflow-routing-20260206.md) — another case of documenting implicit behavior revealing hidden issues
